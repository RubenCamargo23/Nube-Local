...
type Repository interface {
	...
	FindByUsuario(ctx context.Context, usuarioID uuid.UUID) ([]AssignmentResponse, error)
}
...
func (r *repository) FindByProfesor(ctx context.Context, profesorID uuid.UUID) ([]AssignmentResponse, error) {
	rows, err := r.db.Query(ctx,
		`SELECT id, usuario_id, espacio_id, rol, horas_semanales, profesor_id, activa, creado_en
         FROM vinculaciones WHERE profesor_id = $1 ORDER BY creado_en DESC`, profesorID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *repository) FindByUsuario(ctx context.Context, usuarioID uuid.UUID) ([]AssignmentResponse, error) {
	rows, err := r.db.Query(ctx,
		`SELECT id, usuario_id, espacio_id, rol, horas_semanales, profesor_id, activa, creado_en
         FROM vinculaciones WHERE usuario_id = $1 ORDER BY creado_en DESC`, usuarioID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
